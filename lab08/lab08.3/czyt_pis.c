...
_Noreturn void *funkcja_czytelnika(void *);

_Noreturn void *funkcja_pisarza(void *);
...
_Noreturn void *funkcja_czytelnika(void *arg) {
...
_Noreturn void *funkcja_pisarza(void *arg) {
    ...
    for (;;) {
        ...
        usleep(rand() % 3000000);
        printf("pisarz %llu - przed zamkiem\n", pthread_self());

        my_write_lock_lock(czytelnia_p);

        // korzystanie z zasobow czytelni
        printf("pisarz %llu - wchodze\n", pthread_self());

        pisze(czytelnia_p);

        printf("pisarz %llu - wychodze\n", pthread_self());

        my_write_lock_unlock(czytelnia_p);

        printf("pisarz %llu - po zamku\n", pthread_self());
    }
}