...
// Dekompozycja blokowa: każdy proces oblicza część pętli
    int block_size = max_liczba_wyrazow / size;
    int my_start = rank * block_size;
    int my_end = (rank + 1) * block_size - 1;
    int my_stride = 1;

// Ostatni proces oblicza resztę, jeśli max_liczba_wyrazow nie jest podzielne przez size
    if (rank == size - 1) {
        my_end = max_liczba_wyrazow - 1;
    }
...
// Zgromadź wyniki z każdego procesu na procesie zarządzającym (rank == 0)
    double *results = NULL;
    if (rank == 0) {
    ...
    if (rank == 0) {
        // Oblicz sumę wyników z każdego procesu
